...
export default function DriverAvailableOrdersPage() {
  ...
  async function handleAccept(price?: number) {
    ...
    try {
      ...
      toast.success(price ? 'Contraoferta enviada' : '¡Pedido aceptado!')
          ...
          <DialogFooter className="gap-2">
            ...
            {counterPrice && parseFloat(counterPrice) > 0 && (
              ...
              >
                ...
                Contraofertar {formatPrice(parseFloat(counterPrice))}
              </Button>
            )}
            <Button
            ...
            >
              <CheckCircle2 className="h-4 w-4 mr-2" />
              {accepting ? 'Aceptando...' : `Aceptar ${formatPrice(selectedOrder?.proposedPrice || 0)}`}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
